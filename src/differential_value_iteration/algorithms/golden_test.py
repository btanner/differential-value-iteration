...
Tests in here are a bit finicky.

With 32bit precision it is difficult to find a setting where all values
converge nicely. It's also seems odd that DVI and RVI will learn the wrong
values here at step_size=1 when that is the setting that is reliable in the
control problems.
"""
import functools
import itertools
...
_MAKE_RVI = functools.partial(rvi.Evaluation,
                              step_size=.99,  # This can approach 1; fails at 1.
                              # But, hat issue might only be on the multichain.
                              reference_index=0,
                              synchronized=True,
                              )
...
class EvaluationGoldenTest(parameterized.TestCase):
  ...
  def test_correct_values_multichain(self, dtype: np.dtype, alg_constructor,
    ...
    tolerance_places = 3 if dtype is np.float32 else 8
